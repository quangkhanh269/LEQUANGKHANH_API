...
router.post('/add', async(reg, res) =>{
    try {
        const model = new modelDistributor(reg.body)
...
router.get('/getbyid/:id', async(reg, res) =>{
    try {
        const result = await modelDistributor.findById(reg.params.id)
        if(result){
            res.send(result)
        }else{
            res.json({
...
router.patch('/edit/:id', async(reg, res) =>{
    try {
        const result = await modelDistributor.findByIdAndUpdate(reg.params.id,reg.body)
        if(result){
...
router.delete('/delete/:id', async(reg, res) =>{
    try {
        const result = await modelDistributor.findByIdAndDelete(reg.params.id)
        if(result){
            res.json({
                "status":200,
